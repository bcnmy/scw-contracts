...
contract SABasicsTest is SATestBase {
    ...
    function testDeploySAWithDefaultModule() external {
        ...
        assertTrue(sa.isModuleEnabled(address(ecdsaOwnershipRegistryModule)), "module should be enabled");
        assertEq(ecdsaOwnershipRegistryModule.getOwner(address(sa)), alice.addr, "owner should be alice");
        assertEq(address(sa).balance, 1 ether, "smart account should have 1 ether");
        assertEq(token.balanceOf(address(sa)), 1 ether, "smart account should have 1 token");
    }
}
