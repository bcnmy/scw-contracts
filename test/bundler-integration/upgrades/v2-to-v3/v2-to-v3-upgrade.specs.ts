...
describe("Upgrade v2 (Modular ex. Ownerless) to v3 (with Bundler)", async () => {
  let [deployer, smartAccountOwner, alice, charlie, verifiedSigner] =
  ...
  beforeEach(async function () {
    [deployer, smartAccountOwner, alice, charlie, verifiedSigner] =
  ...
  const setupTests = deployments.createFixture(
    async ({ deployments, getNamedAccounts }) => {
      ...
      const BaseImplementationV3 = await ethers.getContractFactory(
        "SmartAccountV3"
      );
      ...
      const expectedSmartAccountAddressV3 =
        await walletFactoryV3.getAddressForCounterFactualAccount(
          ecdsaModule.address,
          ecdsaOwnershipSetupData,
          smartAccountDeploymentIndex
        );

      await walletFactoryV3.deployCounterFactualAccount(
        ecdsaModule.address,
        ecdsaOwnershipSetupData,
        smartAccountDeploymentIndex
      );
  ...
  it("Newly deployed V3 SA and PoC Spending limits module work as intended", async () => {
    const { ecdsaModule, userSAV3, entryPoint, mockToken } = await setupTests();
    ...
    const SpendingLimitsModule = await ethers.getContractFactory(
      "SpendingLimitsModule"
    );
    const spendingLimitsModule = await SpendingLimitsModule.deploy(
      mockToken.address
    );
    ...
    await environment.sendUserOperation(userOp4, entryPoint.address);

    const limitSetData = spendingLimitsModule.interface.encodeFunctionData(
      "setLimits",
    ...
    await environment.sendUserOperation(userOp5, entryPoint.address);
    expect(
      await spendingLimitsModule.getLimits(
        userSAV3.address,
        spenderModule.address
      )
    ).to.equal(ethers.utils.parseEther("10"));

    const spendingTxn = await spenderModule.spend(
      mockToken.address,
      charlie.address,
    ...
    const leftToSpend = (
      ...
      await spendingLimitsModule.calculateSpendingsForPeriod(
        userSAV3.address,
        spenderModule.address
      )
    );
  ...
  it("Can upgrade V2 to V3 and it is upgraded (calls hooks at userOp)", async () => {
    const { ecdsaModule, userSA, entryPoint, mockToken } = await setupTests();
    ...
    const moduleTx = await mockHookModule.transfer(
      userSA.address,
      mockToken.address,
      charlie.address,
      amountToTransfer
    ...
    expect(await mockHookModule.counter()).to.equal(
      mockHookModuleCounterBefore.add(2)
    );
  ...
  it("Can upgrade the not deployed V2 to V3 and it is upgraded", async () => {
    const { entryPoint, smartAccountFactory, ecdsaModule, mockToken } =
      await setupTests();
    ...
    expect(await aliceSA.isModuleEnabled(mockHookModule.address)).to.equal(
      true
    );
  ...
  it("Can upgrade the not deployed V2 to V3 via batched calldata in the userOp: upgrade + send ether", async () => {
    ...
    const execBatchCallData = SmartAccount.interface.encodeFunctionData(
      ...
      [
        [expectedSmartAccountAddress, charlie.address],
        [ethers.utils.parseEther("0"), amountToTransfer],
        [updateImplData, "0x"],
      ]
    );
    ...
    expect(await charlie.getBalance()).to.equal(
      charlieNativeBalanceBefore.add(amountToTransfer)
    );
  });
  ...
  it("Can upgrade the not deployed V2 to V3 via batched calldata in the userOp: upgrade + update callback handler + send erc20 token", async () => {
    ...
    const FallbackHandler = await ethers.getContractFactory(
    ...
    const transferTokenData = encodeTransfer(
      charlie.address,
    ...
    expect(await aliceSA.getImplementation()).to.equal(baseImplV3.address);
    expect(await aliceSA.getFallbackHandler()).to.equal(
      newFallbackHandler.address
    );
    expect(await mockToken.balanceOf(charlie.address)).to.equal(
      charlieTokenBalanceBefore.add(amountToTransfer)
    );
  });
});
