...
import {getEntryPoint, getSmartAccountFactory, getEcdsaOwnershipRegistryModule,getSmartAccountWithModule,getSmartContractOwnershipRegistryModule, getMockToken,deployContract} from "../utils/setupHelper";
...
describe("Smart Contract Ownership Registry Module: ", async()=>{
    ...
    const setupTests = deployments.createFixture( async( {deployments} ) =>{
        ...
        const smartContractOwnershipRegistryModule = await getSmartContractOwnershipRegistryModule();
        const ecdsaRegistryModule = await getEcdsaOwnershipRegistryModule();
        const mockToken = await getMockToken();

        let ecdsaOwnershipSetupData1 = ecdsaRegistryModule.interface.encodeFunctionData(
            "initForSmartAccount",
            [baseSmartAccountOwner1.address]
        );
        let ecdsaOwnershipSetupData2 = ecdsaRegistryModule.interface.encodeFunctionData(
            "initForSmartAccount",
            [baseSmartAccountOwner2.address]
        );
        const smartAccountOwnerContract1= await getSmartAccountWithModule(ecdsaRegistryModule.address, ecdsaOwnershipSetupData1, smartAccountDeploymentIndex);
        const smartAccountOwnerContract2= await getSmartAccountWithModule(ecdsaRegistryModule.address, ecdsaOwnershipSetupData2, smartAccountDeploymentIndex+1);
        ...
        const userSA = await getSmartAccountWithModule(smartContractOwnershipRegistryModule.address, smartContractOwnershipSetupData,smartAccountDeploymentIndex+2);
        ...
        return {
            ...
            saFactory: saFactory,
            smartContractOwnershipRegistryModule: smartContractOwnershipRegistryModule,
    ...
    describe("initForSmartAccount(): ", async()=>{
        it("Should successfully initialize userSA with Smart Contract as Owner", async()=>{
            ...
            const userSA = await getSmartAccountWithModule(smartContractOwnershipRegistryModule.address, smartContractOwnerhsipSetupData,smartAccountDeploymentIndex+1);
        ...
        it("Should revert when setting up EOA as Smart Account Owner", async()=>{
            ...
            const expectedSmartAccountAddress = await saFactory.getAddressForCounterFactualAccount(smartContractOwnershipRegistryModule.address,smartContractOwnershipSetupData,smartAccountDeploymentIndex);
            await expect(saFactory.deployCounterFactualAccount(smartContractOwnershipRegistryModule.address,smartContractOwnershipSetupData,smartAccountDeploymentIndex)).to.be.revertedWith("NotSmartContract");
    ...
    describe("renounceOwnership(): ", async()=>{
        it("Should be able to renounce ownership and the new owner should be address(0)", async()=>{
            ...
            const tx = await entryPoint.handleOps([userOp],alice.address);
            await expect(tx).to.not.emit(entryPoint, "UserOperationRevertReason");
            await expect(smartContractOwnershipRegistryModule.getOwner(userSA.address)).to.be.revertedWith("NoOwnerRegisteredForSmartAccount");
        });
    });
    ...
    describe("validateUserOp(): ", async()=>{
        it("Should return SIG_VALIDATION_SUCCESS for a valid UserOp and valid userOpHash", async()=>{
            ...
            let res = await smartContractOwnershipRegistryModule.validateUserOp(userOp, userOpHash);
        ...
        it("Should revert when userOp.sender is an Unregistered Smart Account", async()=>{
            ...
            const unregisteredSmartAccountAddress = await saFactory.getAddressForCounterFactualAccount(smartContractOwnershipRegistryModule.address,smartContractOwnershipSetupData,smartAccountDeploymentIndex+3);
            await saFactory.deployCounterFactualAccount(smartContractOwnershipRegistryModule.address,smartContractOwnershipSetupData,smartAccountDeploymentIndex+3);
            ...
            const handleOpsTx = await entryPoint.handleOps([renounceOwnershipUserOp],alice.address);
            ...
            await expect(smartContractOwnershipRegistryModule.validateUserOp(sendTokenUserOp,userOpHash)).to.be.revertedWith("NoOwnerRegisteredForSmartAccount");

            const unregisteredSABalanceBefore = await mockToken.balanceOf(unregisteredSA.address);
            await expect(entryPoint.handleOps([sendTokenUserOp],alice.address)).to.be.revertedWith("FailedOp");
            expect(await mockToken.balanceOf(bob.address)).to.equal(bobBalanceBefore);
    ...
    describe("isValidSignatureForAddress(): ", async()=>{
        it("Should return EIP1271_MAGIC_VALUE for valid signature signed by Smart Account Owner",async()=>{
            ...
            const tempSignature = await baseSmartAccountOwner1.signMessage(messageToSign);
        ...
        it("Should return 0xffffffff for signatures not signed by Smart Account Owners ", async()=>{
            ...
            expect(await smartContractOwnershipRegistryModule.isValidSignatureForAddress(dataHash,moduleSignature,userSA.address)).to.equal(EIP1271_INVALID_SIGNATURE);
        });
    });
});
