...
describe("SessionKey: SessionKey Manager Module", async () => {
  ...
  const setupTests = deployments.createFixture(async ({ deployments }) => {
    ...
    return {
      ...
      userSA: userSA,
      danSessionKeyManager: danSessionKeyManager,
  ...
  it("should be enabled", async () => {
    const { userSA, danSessionKeyManager } = await setupTests();
    expect(await userSA.isModuleEnabled(danSessionKeyManager.address)).to.equal(
      true
    );
  });
  ...
  describe("setMerkleRoot", async () => {
    ...
    it("should add new session key to the existing merkle tree", async () => {
      ...
      expect(
        (await danSessionKeyManager.getSessionKeys(userSA.address)).merkleRoot
  ...
  describe("validateSessionKey(): ", async () => {
    it("should revert if wrong leaf is provided", async () => {
      ...
      const validateSesionKeyTxn =
        danSessionKeyManager.callStatic.validateSessionKey(
          userSA.address,
          wrongValidUntil,
          0,
          mockSessionValidationModule.address,
  ...
  describe("isValidSignature", async () => {
    it("should return 0xffffffff even for the valid hash/signature pair", async () => {
      const { danSessionKeyManager } = await setupTests();
      ...
      expect(
        await danSessionKeyManager.isValidSignature(messageHash, signature)
