...
contract AddressResolver {
    ...
    address public constant SA_V1_FACTORY = 0x000000F9eE1842Bb72F6BBDD75E6D3d4e3e9594C;
    address public constant SA_V2_FACTORY = 0x00000016FD385cEE5116EF68C189733679770338;
    address public constant SA_V1_IMPLEMENTATION = 0x00006B7e42e01957dA540Dc6a8F7C30c4D816af5;
    address public constant SA_V2_IMPLEMENTATION = 0x000000988555091db5633a5Be66d563EfB48cB95;
    address public constant ECDSA_REGISTRY_MODULE_ADDRESS = 0x0000001c5b32F37F5beA87BDD5374eB2aC54eA8e;

    // Review returned information
    // Todo Add natspec
    function resolveEOAtoAccountAddresses(address _eoa, uint8 _maxIndex) external view returns (address[] memory) {
    ...
    for (uint i; i < _maxIndex; i++) {
        address v1Address = IAddressResolver(SA_V1_FACTORY).getAddressForCounterFactualAccount(_eoa, i);
        if (v1Address != address(0) && _isSmartContract(v1Address)) {
            _saAddresses[saAddressesIndex] = v1Address;
            saAddressesIndex++;
        }

        bytes4 selector = IEcdsaRegistryModule(ECDSA_REGISTRY_MODULE_ADDRESS).initForSmartAccount.selector;
        bytes memory data = abi.encodeWithSelector(selector, _eoa);

        address v2Address = IAddressResolver(SA_V2_FACTORY).getAddressForCounterFactualAccount(ECDSA_REGISTRY_MODULE_ADDRESS, data, i);
